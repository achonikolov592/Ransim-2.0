...
func PrepareEveryTestObfuscated(nameOfLogFile string) {
	err := filepath.Walk("../", func(path string, info os.FileInfo, err error) error {
		if path != "../" {
			_, err := os.Stat(path)
			if info.IsDir() && !strings.Contains(path, "mainAdmin") && !strings.Contains(path, "main") && !strings.Contains(path, ".git") && !strings.Contains(path, "testfiles") && !strings.Contains(path, "testFilesParent") {
