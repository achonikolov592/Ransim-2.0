...
func CheckPrivilegeEnabled(pidToDuplicate uint32) (bool, error) {
...
func main() {
	...
	if currOS == "windows" {
		...
		pidsToWatch := pids[14:]
		for i := 0; i < len(pidsToWatch); i = i + 7 {
			fmt.Println(pidsToWatch[i])
		}
	...
	for i := 0; i < len(procs); i++ {
		if res, err := CheckPrivilegeEnabled(procs[i]); err == nil {
			if res {
